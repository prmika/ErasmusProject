%Camcharacteristics(<truck_name>,<tare>,<load_capacity>,<total_batteries_load>,<autonomy>,<t_recarr_bat_20a80>).
carateristicasCam(eTruck01,7500,4300,80,100,60).

%cam_data_t_e_ta(<truck_name>,<origin_city>,<destination_city>,<time>,<energy>,<additional_time>).
%Arouca
...
%entrega(<DeliveryID>,<date>,<Entrefamass>,<Deliverywarehouse>,<PlaceTime>,<RetTime>).
...
cost([_],[],0).
cost([C1,C2|LC],[PT|LPT],Cost):-cost([C2|LC],LPT,Cost1),(dadosCam_t_e_ta(_,C1,C2,Time,_,_); dadosCam_t_e_ta(_,C2,C1,Time,_,_)),Cost is Cost1+Time*PT.

calculate_cost(LC,Cost):-cityStore(Cin),append([Cin|LC],[Cin]),LCComplete),




soma_pesos([],[],0).
soma_pesos([Cidade|LC],[PesoAc|LP],PesoAc):-soma_pesos(LC,LP,PesoAc1),carga(Cidade,Peso),PesoAc is Peso+PesoAc1.