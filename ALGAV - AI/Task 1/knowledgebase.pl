...
%Routes
:- http_handler(root(getPlanning), get_planning, []).			

%Get_planning_data
get_planning(Request) :-
    http_parameters(Request,[date(Date,[string])]),
    seq_custo_min(LC,Custo,'20221205'),
    Test = LC,
    format('Content-type: text/plain~n~n'),
	format('~w && ~w && ~w',[Test,Custo,Date]).
...
soma_pesos([],[],0,_).
soma_pesos([Cidade|LC],[PesoAc|LP],PesoAc,Date):-
    soma_pesos(LC,LP,PesoAc1,Date),entrega(_,Date,Peso,Cidade,_,_),PesoAc is Peso+PesoAc1.
...
%c calcula_custo
calcula_custo(LC,Custo,Date):-
    soma_pesos(LC,LP,_,Date),
    tara(Tara),
...
custo([C1,C2|LC],[PT|LPT],Custo):-
    ...
    dadosCam_t_e_ta(_,C1,C2,Time,_,_),
    Custo is Custo1+Time*PT.


%d seq_custo_min

seq_custo_min(LC,Custo,Date):-(run(Date);true),custo_min(LC,Custo).

run(Date):-
    retractall(custo_min(_,_)),
    assertz(custo_min(_,10000000)),
    findall(Cidade,entrega(_,Date,_,Cidade,_,_),LC),
    permutation(LC,LCPerm),
    calcula_custo(LCPerm,Custo,Date),
