...
%delivery(4439, 20221128, 200, 17, 8, 10).
%delivery(4440, 20221128, 150, 14, 7, 9).
%delivery(4441, 20221128, 100, 6, 5, 7).
%delivery(4442, 20221128, 120, 12, 6, 8).
%delivery(4443, 20221128, 300, 13, 15, 20).
%delivery(4444, 20221128, 400, 5, 15, 20).
%delivery(4445, 20221128, 120, 2, 6, 8).

% Trucks
% truckFeatures(name, weight(kg), max_load(kg), full_charge(kWh),
% CurrentBattery(km), time_to_charge(from 20% to 80% in minutes)
...
calculate_cost(LW,Time,LWCharging):-
    truckFeatures(_,TruckWeight,_,CurrentBattery1,_,_),
    CurrentBattery is CurrentBattery1 * 0.8,
    sum_weights(LW,LWe,_),
    add_truck_weight(TruckWeight,LWe,LWeT),
    starting_point(SP),
    append([SP|LW],[SP],LWcomplete),
    cost(LWcomplete,CurrentBattery,Time,LWCharging,LWeT).

cost([_],_,0,[],[]).
cost([W1,W2|LW],CurrentBattery,Time,LWCharging,[WT|LWeT]):-
    truckData(_,W1,W2,T,Energy1,_),
    EnergyDist is Energy1 * WT / 11800,
    ((CurrentBattery < EnergyDist, !,truckFeatures(_,_,_,_,A,TCharge), A1 is A - EnergyDist,LWCharging = [W1|LWPreviousCharges]);
    (A1 is CurrentBattery - EnergyDist,TCharge is 0,LWCharging = LWPreviousCharges)),
    cost([W2|LW],A1,Time1,LWPreviousCharges,LWeT),
    truckFeatures(_,TruckWeight,MaxLoad,_,_,_),
    MaxTotalWeight is TruckWeight + MaxLoad,
    WT1 is WT / MaxTotalWeight,
    TCharge1 is TCharge / 48,
    TCharge2 is 48 - CurrentBattery,
    TCharge3 is TCharge1 * TCharge2,
    starting_point(SP),
    ((W2 =\= SP,delivery(_,_,_,W2,_,UnloadTime),!,Time is TCharge3 + T * WT1 + Time1 + UnloadTime,true);(Time is TCharge3 + T * WT1 + Time1,true)).
% B min_time_seq
